  ...
  return (
    <div className='flex justify-between border-b-2'>
        ...
        <div className='flex gap-4 p-3'>
            ...
            <span className='hidden sm:block'>Create Post </span> <HiPencilAlt className = 'sm:hidden text-20px'/></button>
            {!session ?
            <button onClick={() => signIn()} className='bg-white
             text-gray-500 p-1 px-3 text-[12px] border-[1px]
             rounded-full' >
                <span className='hidden sm:block' >Sign In </span> <HiArrowRight className = 'sm:hidden text-20px'/>  </button>
                 : <button onClick={() => signOut()} className='bg-white
                 text-gray-500 p-1 px-3 text-[12px] border-[1px]
                 rounded-full' >
                    <span className='hidden sm:block' >Sign Out </span> <HiArrowRight className = 'sm:hidden text-20px'/>  </button>
            }
            <Image
               src={session ? (session.user?.image || USER_IMAGE) : USER_IMAGE}
               alt="user-image"
               width={40}
               height={40}
